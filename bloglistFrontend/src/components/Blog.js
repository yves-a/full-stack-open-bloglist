...
const Blog = ({ blog, blogs, setBlogs, update }) => {
  ...
  if (blogState) {
    return (
      <div style={blogStyle}>
        <div>{blog.title}  <button onClick={toggleBlogState}> hide </button>
          <br/>  {blog.url} <br/>
          {blog.likes} <button id="likeButton" onClick={updateLikes}> like </button>
          <br/> {blog.author}
          <br/> <button id="deleteButton" onClick={deleteBlog}> delete </button>
        </div>
      </div>
    )
  }
